...
--   · Al anular un recibo que reabre facturas, y al levantar una insolvencia (desde
--     la aplicación, en la misma transacción).
--   Orden: el recibo más antiguo con saldo, sobre la factura viva más antigua.
--
-- ── 🟡 REGLA PROVISIONAL, pendiente de concepto jurídico (Ley 1116, art. 17) ──
--
-- Aplicar el saldo a favor de un cliente en insolvencia a sus facturas es una
-- compensación. Mientras el abogado o el contador no digan otra cosa (ECM, 2026-09-15):
--   (a) desde `en_insolvencia_desde` la aplicación automática se CONGELA (esta
--       función devuelve 0 con SALDO_A_FAVOR_AUTOMATICO);
--   (b) solo el admin lo mueve, con referencia: COMPENSACION_AUTORIZADA (aplicarlo)
--       o un egreso DEVUELTO_AL_PROCESO;
--   (c) lo aplicado automáticamente entre la fecha de insolvencia y el día en que se
--       marcó no se deshace solo: se lista para revisar (Cartera.estadoDeCuenta);
--   (d) al levantar la insolvencia se descongela y se aplica en esa transacción.
-- Congelar es lo reversible: aplicar no se deshace sin autor.
--
...
ALTER TABLE public.cartera_aplicaciones ADD COLUMN IF NOT EXISTS referencia TEXT   NULL;
...
ALTER TABLE public.cartera_aplicaciones ADD CONSTRAINT ck_aplicaciones_regla CHECK (regla IN (
    ...
    'SALDO_A_FAVOR_AUTOMATICO', 'COMPENSACION_AUTORIZADA', 'DEVOLUCION_DE_SALDO_A_FAVOR'));
...
ALTER TABLE public.cartera_aplicaciones ADD CONSTRAINT ck_aplicaciones_con_autor CHECK (
    (regla <> 'COMPENSACION_AUTORIZADA' OR (referencia IS NOT NULL AND length(btrim(referencia)) > 0 AND usuario_id IS NOT NULL))
    AND (regla <> 'DEVOLUCION_DE_SALDO_A_FAVOR' OR usuario_id IS NOT NULL));
COMMENT ON COLUMN public.cartera_aplicaciones.referencia IS
    'COMPENSACION_AUTORIZADA: la autorizacion (auto del juez, acta del promotor). V74, F4.12.';
...
CREATE OR REPLACE FUNCTION public.fn_aplicar_saldo_a_favor(p_tenant TEXT, p_documento TEXT, p_regla TEXT,
                                                           p_referencia TEXT, p_usuario BIGINT)
...
BEGIN
    IF p_regla NOT IN ('SALDO_A_FAVOR_AUTOMATICO', 'COMPENSACION_AUTORIZADA') THEN
        RAISE EXCEPTION 'El saldo a favor se aplica con SALDO_A_FAVOR_AUTOMATICO o COMPENSACION_AUTORIZADA, no con %.', p_regla;
    END IF;
    -- 🟡 Regla provisional (Ley 1116 art. 17): al cliente en insolvencia no se le compensa solo.
    IF p_regla = 'SALDO_A_FAVOR_AUTOMATICO' AND EXISTS (
    ...
    LOOP
        ...
        WHILE v_queda_fact > 0 LOOP
            ...
            INSERT INTO public.cartera_aplicaciones
                (tenant_id, recibo_id, credito_tx_id, debito_tx_id, monto, regla, referencia, usuario_id)
            VALUES (p_tenant, f.recibo_id, f.credito_tx_id, d.debito_tx_id, v_parte, p_regla, p_referencia, p_usuario);
    ...
    RETURN v_total;
END $$;
COMMENT ON FUNCTION public.fn_aplicar_saldo_a_favor(TEXT, TEXT, TEXT, TEXT, BIGINT) IS
    'Aplica el saldo a favor del cliente a sus facturas vivas (recibo mas viejo sobre factura mas vieja). INVOKER: '
    'RLS aplica. Con SALDO_A_FAVOR_AUTOMATICO no toca a un cliente en insolvencia (regla provisional, Ley 1116 art. 17). V74.';
...
BEGIN
    ...
    ELSE
        ...
        -- vivas más viejas). A un cliente en insolvencia no: su saldo queda congelado (regla
        -- provisional). La función vuelve enseguida si no hay saldo a favor.
        PERFORM public.fn_aplicar_saldo_a_favor(NEW.tenant_id, NEW.cliente_documento, 'SALDO_A_FAVOR_AUTOMATICO', NULL, NULL);
    END IF;
...
BEGIN
    ...
    END LOOP;
    REVOKE EXECUTE ON FUNCTION public.fn_aplicar_saldo_a_favor(TEXT, TEXT, TEXT, TEXT, BIGINT) FROM PUBLIC;
    IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'app_user') THEN
        ...
        GRANT EXECUTE ON FUNCTION public.fn_aplicar_saldo_a_favor(TEXT, TEXT, TEXT, TEXT, BIGINT) TO app_user;
    END IF;
END
$rls$;
...
BEGIN
    ...
    -- Regla provisional: el insolvente con saldo a favor compra (desde la caja, queda marcada) y el saldo NO se aplica.
    INSERT INTO public.recibos_de_caja (tenant_id, numero, cliente_documento, monto, medio, ocurrido_en, idempotency_key)
    ...
    IF (SELECT saldo FROM public.v_cartera_por_documento WHERE tenant_id = a AND order_uuid = v_ins) <> 40000
       ...
       OR public.fn_aplicar_saldo_a_favor(a, 'v74-ins', 'SALDO_A_FAVOR_AUTOMATICO', NULL, NULL) <> 0 THEN
        RAISE EXCEPTION 'V74: el saldo a favor de un cliente en insolvencia se aplico solo';
    END IF;
    -- (b) Compensación autorizada: sin referencia no entra; con referencia, sí.
    v_rechazado := false;
    BEGIN
        PERFORM public.fn_aplicar_saldo_a_favor(a, 'v74-ins', 'COMPENSACION_AUTORIZADA', NULL, v_user);
    EXCEPTION WHEN check_violation THEN v_rechazado := true;
    END;
    IF NOT v_rechazado THEN RAISE EXCEPTION 'V74: una compensacion sin referencia entro'; END IF;
    v_n := public.fn_aplicar_saldo_a_favor(a, 'v74-ins', 'COMPENSACION_AUTORIZADA', 'Auto 123 del juez', v_user);
    IF v_n <> 15000 OR (SELECT saldo FROM public.v_cartera_por_documento WHERE tenant_id = a AND order_uuid = v_ins) <> 25000 THEN
        RAISE EXCEPTION 'V74: la compensacion autorizada aplico % y no 15000', v_n;
    END IF;
    ...
    v_rechazado := false;
    BEGIN
        PERFORM public.fn_aplicar_saldo_a_favor(a, 'v74-i', 'DEVOLUCION_DE_SALDO_A_FAVOR', NULL, v_user);
    EXCEPTION WHEN raise_exception THEN v_rechazado := true;
    END;
    IF NOT v_rechazado THEN RAISE EXCEPTION 'V74: fn_aplicar_saldo_a_favor acepto una regla que no es suya'; END IF;
    ...
    END IF;
    IF has_function_privilege('public', 'public.fn_aplicar_saldo_a_favor(text, text, text, text, bigint)', 'EXECUTE') THEN
    ...
    IF (SELECT prosecdef FROM pg_proc WHERE oid = to_regprocedure('public.fn_venta_a_credito()'))
       OR (SELECT prosecdef FROM pg_proc WHERE oid = to_regprocedure('public.fn_aplicar_saldo_a_favor(text, text, text, text, bigint)')) THEN
    ...
    RAISE NOTICE 'V74: (i) 120.000 sobre 100.000 deja 20.000 a favor y la venta de 50.000 debe 30.000; (ii) devolver 20.000 deja favor 0 y libro 0; insolvente congelado; compensacion solo con referencia; egreso solo anexa; INVOKER; 0 restos';
END $cierre$;
...
-- DROP FUNCTION public.fn_aplicar_saldo_a_favor(TEXT, TEXT, TEXT, TEXT, BIGINT); DROP VIEW public.v_saldo_a_favor_por_recibo;
...
-- CHECK de cartera_aplicaciones: el de V64; DROP COLUMN referencia, usuario_id; DROP INDEX ix_debt_transactions_recibo.
